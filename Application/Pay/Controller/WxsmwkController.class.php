...
class WxsmwkController extends PayController{
	...
	public function Pay($array){
        ...
        $notifyurl = $this->_site . 'Pay_Wxsmwk_notifyurl.html';
        //$callbackurl = $this->_site . 'Pay_Wxsmwk_callbackurl.html';
        ...
        if($result['return_msg'] == 'OK' && $result['return_code'] == 'SUCCESS' && $result['result_code'] == 'SUCCESS' && $result["code_url"]){
            import("Vendor.phpqrcode.phpqrcode",'',".php");
            $url = $result['code_url'];
            $QR = "Uploads/codepay/". $result['out_trade_no'] . ".png";//已经生成的原始二维码图
            \QRcode::png($url, $QR, "L", 20);
            $this->assign("imgurl", '/'.$QR);
            $this->assign('params',$result);
            $this->assign('orderid',$result['out_trade_no']);
            $this->assign('money',sprintf('%.2f',$result['total_fee']/100));
            $this->display("WeiXin/weixinklt");
        }else{
    ...
    public function notifyurl()//文档没有签名 以后看到的人自己加
    {
        ...
        if( $this->inst_id == $parameter['inst_id'] && 'SUCCESS' == $parameter['return_code'] && $parameter['out_trade_no'])
        {
            if($data['status'] == 0 && $respSign == $sign){
                $this->EditMoney($parameter["out_trade_no"], 'Wxsmwk', 0);
                exit('success');
            }
